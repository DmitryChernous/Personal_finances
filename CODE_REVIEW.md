...
### 1. Мертвый код: `pfProcessFileImport_()` не используется ✅ ИСПРАВЛЕНО

**Местоположение:** `src/Import.js:1055-1118`
...
- Тело функции заменено на `throw new Error()` с объяснением
- Функция будет выбросить ошибку, если случайно будет вызвана

**Приоритет:** Средний (не влияет на работу, но засоряет код)

---

### 2. Мертвый код: `pfProcessImportData_()` не используется ✅ ИСПРАВЛЕНО

**Местоположение:** `src/Import.js:288-295`
