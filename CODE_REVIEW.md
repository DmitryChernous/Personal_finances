...
### 1. Мертвый код: `pfProcessFileImport_()` не используется

**Местоположение:** `src/Import.js:1172-1210`

**Проблема:**
Функция `pfProcessFileImport_()` определена, но нигде не вызывается. Вся логика импорта переписана на использование публичных функций (`pfDetectFileFormat`, `pfParseFileContent`, `pfProcessDataBatch`, `pfWritePreview`), которые вызываются из клиентского кода в `ImportUI.html`.

**Решение:**
- Удалить функцию `pfProcessFileImport_()` или пометить как deprecated
- Если планируется использовать в будущем, добавить комментарий с объяснением

**Приоритет:** Средний (не влияет на работу, но засоряет код)

---

### 2. Мертвый код: `pfProcessImportData_()` не используется

**Местоположение:** `src/Import.js:262-318`

**Проблема:**
Функция `pfProcessImportData_()` определена, но не вызывается нигде в коде. Вся обработка данных происходит в `pfProcessDataBatch()`.

**Решение:**
- Удалить функцию или оставить как пример/справочник (с пометкой)

**Приоритет:** Средний

---

### 3. Хардкод строки в меню (нарушение i18n)
...
**Решение:**
- Добавить ключ в `src/I18n.js`:
  ```javascript
  find_duplicate: 'Найти дубликат (по ключу)'
  ```
- Использовать `pfT_('menu.find_duplicate')` в меню
...
### 4. Избыточное логирование в production коде

**Местоположение:** Множественные файлы, особенно `src/Import.js` (120+ вызовов `Logger.log`)
...
**Примеры:**
- `src/Import.js`: ~120 вызовов `Logger.log`
- `src/Template.js`: ~23 вызова `Logger.log`
- Многие логи дублируют информацию

**Решение:**
- Создать систему уровней логирования (DEBUG, INFO, WARNING, ERROR)
- Оставить только критичные логи (ERROR, WARNING)
- Удалить или закомментировать DEBUG логи
- Использовать условную компиляцию или флаги для включения/выключения логирования

**Приоритет:** Средний

---

### 5. Дублирование логики генерации ключей дедупликации

**Местоположение:** 
- `src/Import.js:pfGenerateDedupeKey_()` (строки 76-130)
- `src/Import.js:pfFindDuplicateTransaction()` (строки 580-600)
- `src/Import.js:pfGetExistingTransactionKeys_()` (строки 320-380)

**Проблема:**
Логика генерации ключей дедупликации дублируется в трех местах с небольшими вариациями. Это приводит к:
...
**Решение:**
- Вынести общую логику в единую функцию `pfGenerateDedupeKeyFromRow_(row, sourceCol, sourceIdCol, dateCol, accountCol, amountCol, typeCol)`
- Использовать эту функцию везде, где нужна генерация ключа
...
### 7. Смешение ответственности в модулях

**Местоположение:** 
- `src/Setup.js` - содержит логику Help (строки 347-505)
- `src/Import.js` - содержит слишком много ответственности (DTO, импорт, дедупликация, preview, commit)

**Проблема:**
Модули делают слишком много разных вещей, что усложняет поддержку и тестирование.

**Решение:**
- Вынести Help content в отдельный модуль `Help.js`
- Разбить `Import.js` на:
...
**Решение:**
- Создать модуль `Constants.js` с константами:
  ```javascript
...
**Список:**
1. `pfProcessFileImport_()` - `src/Import.js:1172`
2. `pfProcessImportData_()` - `src/Import.js:262`

**Решение:**
- Удалить или пометить как deprecated с комментарием о причине
...
### 10. Дублирование логики очистки sheet

**Местоположение:**
- `src/Import.js:pfPreviewImport_()` (строки 391-407)
- `src/Import.js:pfCommitImport_()` (строки 791-809)
- `src/Template.js:pfPrepareTemplate_()` (множественные места)

**Проблема:**
Логика безопасной очистки sheet (clearContent, clearFormat, clearNote, deleteRows с fallback) дублируется в трех местах.

**Решение:**
- Создать функцию `pfClearSheetRows_(sheet, startRow, numRows)` в `src/Sheets.js`
- Использовать везде
...
### 15. Отсутствие валидации входных данных в публичных функциях

**Местоположение:** 
- `src/Import.js:pfCommitImport()` - нет проверки типов
- `src/Template.js:pfCreateTemplate()` - нет проверки состояния таблицы

**Проблема:**
Публичные функции не всегда валидируют входные данные, что может привести к ошибкам.

**Решение:**
- Добавить валидацию входных данных во все публичные функции
- Использовать единый формат валидации
