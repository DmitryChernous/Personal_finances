...
- `src/Setup.js` — идемпотентная инициализация (листы, шапки, валидации, именованные диапазоны)
...
- **Этап 2 (UX для транзакций) завершён**: валидация, нормализация, подсветка ошибок, пометка на проверку.
- Следующий шаг разработки: **Этап 3** — базовая аналитика (метрики, отчёты).
