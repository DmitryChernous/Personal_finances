...
- [ ] Утвердить колонки `Transactions` (финальный список + форматы)
- [ ] Реализовать `pfSetup()` как идемпотентную инициализацию:
  - [ ] Создание листов `Settings`, `Help`, `Dashboard`, `Reports` (минимально)
  - [ ] Шапки таблиц, frozen row, автофильтр
  - [ ] Data validation: `Type`, `Account`, `Category`, `Currency`
  - [ ] Именованные диапазоны для справочников
- [ ] Версионирование схемы в `Settings` (например `SchemaVersion`)
...
- [ ] KPI: доход/расход/баланс за месяц, средние траты, отклонения
- [ ] Графики: расходы по категориям (pie/bar), динамика по месяцам (line), cashflow
- [ ] UX: переключатель периода, компактный и понятный вид
