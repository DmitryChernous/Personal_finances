...
## Сценарий: новый пользователь и raw-листы (синхронизация raw)

- [ ] Запустите **Personal finances → Setup (создать листы)**.
- [ ] Создайте лист с именем `raw_Тест` (или аналогичным). Первая строка — заголовки: ДАТА, ВРЕМЯ, КАТЕГОРИЯ, ОПИСАНИЕ, СУММА, ОСТАТОК СРЕДСТВ, СЧЕТ.
- [ ] Во второй и третьей строках добавьте 2–3 тестовые транзакции (дата dd.mm.yyyy, сумма с минусом или плюсом).
- [ ] Запустите **Personal finances → Синхронизировать с raw-листами**.
- [ ] Проверьте: в листе «Транзакции» появились новые строки; колонки Дата, Сумма, ID источника отформатированы; количество добавленных совпадает с сообщением.
- [ ] Запустите синхронизацию ещё раз. Ожидаемо: «Добавлено: 0», «Пропущено (уже в Транзакциях): N» (N = число строк с raw-листа). Новых строк в «Транзакции» не должно появиться.
- [ ] Запустите **Обновить отчёты** и **Обновить дашборд**. Убедитесь, что данные отображаются без ошибок.

## Результаты тестирования

✅ **2026-01-24**: Все тесты Этапа 2 пройдены успешно, ошибок не обнаружено.

## Тестирование Этапа 3 — Базовая аналитика

### Тест 10: Инициализация листа Reports

- [ ] Запустите **Personal finances → Setup (создать листы)**
- [ ] Проверьте, что лист `Отчеты` / `Reports` создан
- [ ] Проверьте, что на листе есть секции:
  - "Сводка по периодам" / "Summary by Period"
  - "Топ расходов по категориям" / "Top Expenses by Category"
  - "Денежный поток по месяцам" / "Monthly Cashflow"
  - "Остатки по счетам" / "Account Balances"

### Тест 11: Доходы/расходы за период

- [ ] Добавьте несколько транзакций в лист `Транзакции`:
  - Доходы (`Type = income`) за текущий месяц
  - Расходы (`Type = expense`) за текущий месяц
  - Транзакции за текущий год
- [ ] Перейдите на лист `Отчеты`
- [ ] Проверьте секцию "Сводка по периодам":
  - В строке "Текущий месяц" должны быть суммы доходов и расходов
  - В строке "Текущий год" должны быть суммы доходов и расходов
  - Колонка "Итого" должна показывать разницу (доходы - расходы)
- [ ] Проверьте, что числа отображаются корректно (формат `#,##0.00`)

### Тест 12: Топ расходов по категориям

- [ ] Убедитесь, что в `Транзакции` есть расходы (`Type = expense`) с заполненными категориями за текущий месяц
- [ ] На листе `Отчеты` проверьте секцию "Топ расходов по категориям"
- [ ] Должна быть таблица с колонками "Категория" и "Сумма"
- [ ] Категории должны быть отсортированы по убыванию суммы
- [ ] Должно быть не более 10 категорий

### Тест 13: Остатки по счетам

- [ ] Убедитесь, что в листе `Счета` есть несколько счетов
- [ ] Добавьте транзакции для этих счетов (доходы и расходы)
- [ ] На листе `Отчеты` проверьте секцию "Остатки по счетам"
- [ ] Для каждого счёта должна быть формула, которая считает: доходы - расходы
- [ ] Проверьте, что остатки отображаются корректно

### Тест 14: Обновление отчётов

- [ ] Добавьте новую транзакцию в `Транзакции`
- [ ] Запустите **Personal finances → Обновить отчёты**
- [ ] Проверьте, что отчёты обновились (суммы изменились)
- [ ] Проверьте, что структура листа `Отчеты` не сломалась

### Тест 15: Автоматическое обновление формул

- [ ] Добавьте/измените транзакции в `Транзакции`
- [ ] Перейдите на лист `Отчеты` (без запуска "Обновить отчёты")
- [ ] Проверьте, что формулы автоматически пересчитались (Google Sheets делает это автоматически)
- [ ] Суммы должны обновиться сразу

### Тест 16: Локализация отчётов

- [ ] Переключите язык на английский: **Personal finances → Язык → English**
- [ ] Проверьте, что лист `Reports` переименовался
- [ ] Проверьте, что заголовки секций на английском
- [ ] Переключите обратно на русский — всё должно вернуться

### Известные ограничения Этапа 3

- Месячный cashflow (динамика по месяцам) пока не реализован — есть placeholder
- Остатки по счетам не учитывают начальный баланс из листа `Счета` (только транзакции)
- Остатки по счетам не учитывают переводы (`transfer`) — только доходы и расходы
- Формулы могут быть медленными при большом количестве транзакций (1000+)
