...
**ВАЖНО:** После Setup лист "Транзакции" пустой. Нужно сгенерировать тестовые данные для тестирования бюджетов.

- [ ] Запустить меню: **Personal finances → Заполнить тестовыми данными**
- [ ] В диалоге выбрать **"Да"** (очистить существующие данные) или **"Нет"** (добавить к существующим)
- [ ] Проверить, что созданы:
  - **5 счетов:** Наличные, Карта Сбер, Карта Тинькофф, Вклад, Доллары
  - **15 категорий:** Еда (Продукты, Рестораны), Транспорт (Общественный, Бензин), Жильё, Развлечения, Здоровье, Одежда, Зарплата и др.
  - **~60 транзакций** за последние 3 месяца (включая текущий месяц)
- [ ] Проверить лист "Транзакции" — должны быть транзакции с категориями и суммами

---

## Тест 1: Создание бюджета

**Перед началом:** Убедитесь, что выполнены шаги 1-3 из "Подготовка к тестированию" (Setup + генерация тестовых данных).

### Шаги:
1. Перейти на лист **"Бюджеты"**
2. Добавить новую строку с данными:
   - **Категория:** Еда (должна быть из справочника - выберите из выпадающего списка)
   - **Подкатегория:** Продукты (опционально)
   - **Период:** month (выберите из списка: month или year)
   - **Значение периода:** 2026-01 (текущий месяц в формате YYYY-MM, например, январь 2026 = "2026-01")
   - **Сумма:** 10000
   - **Активно:** оставить пустым (по умолчанию true)
...
- ✅ Выполнена генерация тестовых данных (шаг 3 из подготовки)
- В листе **"Транзакции"** есть транзакции с категорией "Еда" и подкатегорией "Продукты" за текущий месяц
...
1. Создать бюджет:
   ...
   - **Период:** month
   - **Значение периода:** 2026-01 (текущий месяц)
   - **Сумма:** 10000
2. В меню выбрать: **Personal finances → Обновить бюджеты**

### Ожидаемый результат:
- ✅ Колонка **"Факт"** заполнена суммой всех транзакций категории "Еда" / подкатегории "Продукты" за текущий месяц
...
- ✅ Форматирование: "Факт" и "Остаток" как числа с 2 знаками, "% использования" как процент
- ✅ Если факт > 0, значит транзакции найдены и учтены
