...
function pfEnsureHelpContent_(ss) {
  ...
  if (lang === 'en') {
    sheet.getRange(1, 1).setValue('Personal finances — Help');
    sheet.getRange(3, 1).setValue('Quick start:');
    sheet.getRange(4, 1).setValue('1) Run "Setup" from the menu.');
    sheet.getRange(5, 1).setValue('2) Fill Accounts and Categories.');
    sheet.getRange(6, 1).setValue('3) Add Transactions and review Dashboard/Reports.');
  } else {
    sheet.getRange(1, 1).setValue('Personal finances — Инструкция');
    sheet.getRange(3, 1).setValue('Быстрый старт:');
    sheet.getRange(4, 1).setValue('1) В меню запусти "Setup (создать листы)".');
    sheet.getRange(5, 1).setValue('2) Заполни "Счета" и "Категории".');
    sheet.getRange(6, 1).setValue('3) Добавляй "Транзакции" и смотри "Дашборд/Отчеты".');
  }
